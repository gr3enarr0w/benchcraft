"""Real-dataset validation for benchcraft_lazyforecast.
...
schema `benchcraft_lazyforecast` expects, then run through the package's
...
from benchcraft_lazyforecast import ForecastConfig, backtest, forecast, prepare_frame, validate_input


def _load_co2_monthly_panel() -> pd.DataFrame:
    """Real Mauna Loa CO2 concentration, resampled to monthly means.

    Reshaped into the unique_id/ds/y long format `benchcraft_lazyforecast`
