"""Tests for `dscraft.vision.pipeline.SimpleImagePipeline`.
...
from dscraft.vision import PipelineConfig, SimpleImagePipeline
...
def test_pipeline_is_a_dense_media_pipeline_subclass() -> None:
    ...
    `dscraft.core.data.DenseMediaPipeline`, not a parallel interface, per
    CLAUDE.md's "fix what's there, don't duplicate" rule."""
    # Per CLAUDE.md's "fix what's there, don't duplicate" rule: this must
    # be a real subclass of dscraft.core.data.DenseMediaPipeline, not a
    # parallel interface.
...
def test_to_dense_tensor_satisfies_dlpack_protocol() -> None:
    ...
    # dscraft.core.data.DenseMediaPipeline.to_dense_tensor's contract requires
