...
import dscraft.clean.embeddings as embeddings_module
from dscraft.clean import MODEL_ALLOWLIST, RECOMMENDED_MODEL_NAME
from dscraft.clean.embeddings import (
    EmbeddingModel,
...
def test_no_pytorch_or_transformers_imported():
    ...
    This used to also assert ``"torch" not in sys.modules`` /
    ``"transformers" not in sys.modules``, but that is a whole-*process*
    check, not a ``dscraft.clean``-specific one: now that all nine
    subpackages' test suites run together in a single combined pytest
    process (see packages/dscraft/tests), other subpackages that
    legitimately and intentionally depend on torch/transformers
    (``dscraft.tune``, ``dscraft.vision``, ``dscraft.graph``) import them as
    part of their own tests, populating ``sys.modules`` for the rest of the
    process regardless of test file ordering. That is not a violation of
    this package's PyTorch-free constraint -- ``dscraft.clean`` itself still
    never imports either module -- so asserting on process-global
    ``sys.modules`` here produces a false failure purely as an artifact of
    which other tests happened to run first in the same process, not a real
    regression in this package's code.

    The real guarantee -- that no file *under dscraft.clean itself* ever
    imports torch/transformers, called or not -- is the static source scan
    below, which is unaffected by what any other subpackage's tests do in
    the same process. That is a strictly stronger check than the removed
    ``sys.modules`` assertions ever were (per this test's own original
    docstring: ``sys.modules`` "would silently miss a deferred import
    stashed inside a rarely-called function that this test suite happens
    not to exercise").
    """
    ...
    assert offending == [], (
        ...
        "dscraft.clean is deliberately PyTorch-free -- see the "
