"""Real tests for dscraft.core.sandbox.seatbelt.SeatbeltSandboxExecutor.
...
from dscraft.core.sandbox.base import SandboxPolicy
from dscraft.core.sandbox.seatbelt import SeatbeltSandboxExecutor, build_sbpl_profile
...
def _install_no_subprocess_no_tempdir_tripwires(monkeypatch, *, reason: str):
    ...
    (as seen from ``dscraft.core.sandbox.seatbelt``) to raise ``AssertionError``
    if called.
    ...
    monkeypatch.setattr(
        "dscraft.core.sandbox.seatbelt.subprocess.run", _fail_if_subprocess_run_called
    )
    monkeypatch.setattr(
        "dscraft.core.sandbox.seatbelt.tempfile.TemporaryDirectory",
