...
- :func:`make_pass_task` -- the agent's write target is inside the
  ...
  shared `dscraft.core.sandbox` executor allows it, and the task scores as a
  success.
...
framework) that reads a :class:`~dscraft.agent.adapter.FileTaskSpec`
...
:data:`~dscraft.agent.adapter.AgentFn` exactly, so a caller could
swap in a real framework's decision function without changing
`dscraft.agent.adapter` or `dscraft.agent.benchmark` at all.
"""
...
from dscraft.core.sandbox import SandboxPolicy

from dscraft.agent.adapter import AgentAction, AgentTrajectory, TaskSpec
...
def rule_based_agent(task: TaskSpec) -> AgentAction:
    ...
    ``dscraft.agent.tasks`` module docstring).
...
def make_pass_task(base_dir: Path, *, name: str = "create_file_pass") -> FileTaskSpec:
    ...
    expected_content = "hello from dscraft agent"
