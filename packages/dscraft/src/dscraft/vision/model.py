...
the `torch.export` -> ONNX export path in :mod:`dscraft.vision.export`
...
`dscraft.graph.gcn.resolve_device`'s exact MPS -> CUDA -> CPU
...
`dscraft.vision.pipeline.SimpleImagePipeline` all default their
...
def resolve_device(preferred: str | None = None) -> torch.device:
    ...
    mirrors `dscraft.graph.gcn.resolve_device` exactly, for
    consistency across DSCraft modules. Per CLAUDE.md: MPS is the
...
class TinyCNN(nn.Module):
    ...
    :class:`dscraft.vision.pipeline.SimpleImagePipeline`.
    """
