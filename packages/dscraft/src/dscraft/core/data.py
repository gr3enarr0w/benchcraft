...
dscraft.core does not ship a data-processing library. It documents and
lightly implements the three-tier convention that every dscraft subpackage agrees to
...
Tier 1 - Dense tabular / text / time-series
    ...
    pandas nor polars is a hard dependency of ``dscraft.core`` -- consumers
...
Tier 2 - Sparse graph tensors
    ...
    adapter is required. dscraft.core only defines the *shape* of that adapter
    ...
    in LazyGraph, not here -- dscraft.core never depends on torch, PyG, or DGL.
...
def _require_pandas() -> "pd":  # pragma: no cover - trivial import guard
    try:
        ...
        raise ImportError(
            "This dscraft.core.data helper requires pandas to be installed in "
            "the calling module's environment. dscraft.core itself does not "
            "depend on pandas."
        ) from exc
    return pd
...
def _require_polars() -> "pl":  # pragma: no cover - trivial import guard
    try:
        ...
        raise ImportError(
            "This dscraft.core.data helper requires polars to be installed in "
            "the calling module's environment. dscraft.core itself does not "
            "depend on polars."
        ) from exc
    return pl
...
class SparseGraphTensorAdapter(abc.ABC):
    ...
    CSR/CSC) is unavoidable. dscraft.core defines only the *shape* of that
...
class _SupportsDLPack(Protocol):
    ...
    protocol, without dscraft.core needing to depend on torch/numpy to say so.
    """
...
class DenseMediaPipeline(abc.ABC):
    ...
    *compute*, not by data-copy/serialization overhead (this is why FFCV
    ...
    dscraft.core does not implement decode/augment logic and does not depend on
