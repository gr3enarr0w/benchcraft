...
or red-team target -- so dscraft.core provides one executor interface and one
...
class SandboxPolicy:
    ...
    field values -- dscraft.core does not encode any module-specific policy
    ...
    **Path fields must be absolute.** ``allowed_read_paths``,
    ...
    time* (inside :func:`~dscraft.core.sandbox.seatbelt.build_sbpl_profile`,
    ...
    Attributes:
        ...
        allowed_read_paths: Filesystem paths (files or directories) the
            ...
            e.g. :func:`~dscraft.core.sandbox.seatbelt.build_sbpl_profile`).
...
class SandboxResult:
    ...
    #: :mod:`dscraft.core.sandbox.seatbelt` for the exact mechanism and its
...
class BaseSandboxExecutor(abc.ABC):
    ...
    Concrete backends (:class:`~dscraft.core.sandbox.seatbelt.SeatbeltSandboxExecutor`
    on macOS, :class:`~dscraft.core.sandbox.linux_stub.LinuxNamespaceSandboxExecutor`
    ...
    def run_callable(
        ...
        **Backends must never pickle (or otherwise deserialize-on-the-way-in)
        ...
        :class:`~dscraft.core.sandbox.seatbelt.SeatbeltSandboxExecutor`'s
