...
dscraft.core adopts OpenTelemetry GenAI semantic conventions as the shared
...
``dscraft.core`` import-safe and dependency-light for every module, while still
...
_DEFAULT_TRACER_NAME = "dscraft.core"
...
def get_tracer(instrumenting_module_name: str | None = None) -> Tracer:
    ...
    goes through one dscraft.core entrypoint rather than importing
...
def genai_span(
    ...
    *,
    ...
    ``tracer`` defaults to :func:`get_tracer` with the shared dscraft.core
...
def add_transcript_event(
    ...
    **Why no default hash.** An earlier version of this function attached
    ...
    dscraft.core is still scaffold-depth, no module currently needs
    ...
    the recorded content; callers in other dscraft subpackages (security,
    agent) must make the same explicit choice deliberately once they
