"""Runnable end-to-end demo of dscraft.vision's signature capability.

Per CLAUDE.md's "no net-new scripts" rule: this example imports and calls
the real package API (`dscraft.vision`) rather than reimplementing
...
Flow:
    ...
    6. Repeat steps 1-4 on a *real* handwritten-digit image from
       ...
       `dev` extra (`pip install -e "packages/dscraft[dev]"`) since
       scikit-learn is a dev/test-only dependency, not a core runtime one.

Run with:
    python packages/dscraft/examples/vision/export_cnn_example.py
"""
...
def main() -> None:
    print("=== dscraft-vision: pipeline + torch.export -> ONNX demo ===\n")
