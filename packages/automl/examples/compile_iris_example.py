"""Runnable demo of benchcraft_automl's `.compile()` capability on Iris.
...
`benchcraft_automl.compile`, runs inference through `onnxruntime`, and
...
Requires the `onnx` extra:
    pip install -e "packages/automl[onnx]"
"""
