...
1. **License Isolation Policy ("LazyIsolate", §2.2)** -- a decision table
   ...
   that table. It is documentation encoded as data -- lazycore does not
...
per-module maintenance task -- not something lazycore does. Every
...
class Allowlist:
    ...
    Each Benchcraft module owns its own ``Allowlist`` instance (e.g.
    LazyForecast's allowlist listing TimesFM/Chronos-Bolt as Tier 1 and
    MOIRAI as Tier 2). ``lazycore`` provides the mechanism only; every
