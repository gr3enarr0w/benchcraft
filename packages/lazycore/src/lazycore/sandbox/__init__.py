...
per §2.3, LazyCore provides **one** shared sandbox executor interface and
...
network egress) is ever constrained. See :mod:`lazycore.sandbox.seatbelt`'s
...
def get_default_executor(policy: SandboxPolicy | None = None) -> BaseSandboxExecutor:
    ...
    if system == "Darwin":
        ...
        raise SandboxBackendUnavailableError(
            ...
            "implemented in lazycore.sandbox."
        )
    ...
    raise SandboxBackendUnavailableError(
        ...
        "lazycore.sandbox only implements macOS (Seatbelt) and a "
