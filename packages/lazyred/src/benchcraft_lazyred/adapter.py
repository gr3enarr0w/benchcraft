...
``BaseSecurityAdapter`` pattern. Nothing in `lazycore` defines this base
...
``benchcraft_lazytune.adapter.BaseTrainingAdapter`` used for LazyTune's
Adapter-Factory pattern.

Only one concrete adapter is implemented against this interface in this
pass: :class:`benchcraft_lazyred.probes.PromptInjectionAdapter` (see that
...
- :class:`Attempt` -- garak's "attempt" transaction object, scaled down to
  ...
  :class:`~lazycore.sandbox.SandboxResult` from actually invoking the
...
- :class:`Finding` -- one probe attempt's verdict, mapped to an OWASP
  ...
  detail to be reported via :mod:`lazycore.telemetry` and aggregated by
  :mod:`benchcraft_lazyred.leaderboard`.
...
:class:`~lazycore.sandbox.BaseSandboxExecutor` (the shared executor from
`lazycore.sandbox`) and is responsible only for supplying a LazyRed-shaped
:class:`~lazycore.sandbox.SandboxPolicy` on top of it. See
...
class Attempt(object):
    ...
    :class:`~lazycore.sandbox.SandboxResult` from running the target
...
class Finding:
    ...
    Reported via :mod:`lazycore.telemetry`'s GenAI-schema OTel helpers
    (§2.6) rather than a parallel reporting schema -- see
    :func:`benchcraft_lazyred.probes.PromptInjectionAdapter.detect` for
...
class BaseSecurityAdapter(abc.ABC):
    ...
    2. :meth:`run_target` -- actually invoke the target, through the
       shared :mod:`lazycore.sandbox` executor, and record its raw output
    ...
    interface" rule): :class:`benchcraft_lazyred.probes.PromptInjectionAdapter`.
    """
